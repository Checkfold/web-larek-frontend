...
#### GodObject (Представитель) - посредник между Model и View. Содержит всю бизнес-логику, обрабатывает события от View, обновляет Model и обновляет View. Реализован в index.ts.
...
12. `order.payment:field-change`
Когда срабатывает: при изменении способа оплаты в форме заказа.
Аргументы: { field: keyof FormErrors, value: paymentMethod } — имя поля и выбранный способ оплаты.
Назначение: обновляет данные заказа и отображение формы.

13. `order.address:field-change`
Когда срабатывает: при изменении адреса доставки в форме заказа.
Аргументы: { field: keyof FormErrors, value: string } — имя поля и введенный адрес.
Назначение: обновляет данные заказа и отображение формы.

14. `contacts.email:field-change`
Когда срабатывает: при изменении email в форме контактов.
Аргументы: { field: keyof FormErrors, value: string } — имя поля и введенный email.
Назначение: обновляет данные заказа и отображение формы.

15. `contacts.phone:field-change`
Когда срабатывает: при изменении телефона в форме контактов.
Аргументы: { field: keyof FormErrors, value: string } — имя поля и введенный телефон.
Назначение: обновляет данные заказа и отображение формы.

16. `errors:updated`
Когда срабатывает: при изменении состояния ошибок валидации форм.
Аргументы: { field: keyof FormErrors } — поле, для которого обновились ошибки.
Назначение: обновляет отображение ошибок валидации и состояние кнопок отправки форм в зависимости от валидности данных.
...
order: IOrder;              // Данные текущего заказа (email, phone, address, payment)
...
checkField(): boolean
```
Проверяет конкретное поле заказа на валидность, обновляет ошибки формы и эмитит 'errors:updated'.
...
## Класс Form<T>
Назначение класса: Класс Form<T> является базовым компонентом для работы с HTML-формами. Наследуется от Component<IFormState> и предоставляет общую функциональность для обработки пользовательского ввода, управления состоянием кнопки отправки и отображения ошибок. Реализует механизм событий для уведомления об изменениях полей формы.
...
protected events: IEvents;               // Система событий для коммуникации
```
...
```
Назначение: Инициализирует экземпляр класса Form. Находит элементы кнопки отправки и контейнера ошибок, настраивает обработчики событий ввода и отправки формы. Принимает параметрами: `container - HTMLFormElement`, корневой элемент формы, `events` - экземпляр системы событий для коммуникации между компонентами
...
```
Назначение: Управляет состоянием кнопки отправки формы. Устанавливает атрибут disabled на кнопке в зависимости от валидности формы. Принимает параметром: `value: boolean` - флаг валидности формы

2. Сеттер errors: string
```
set errors(value: string)
```
Назначение: Устанавливает текст ошибок валидации в контейнер ошибок. Принимает параметром: `value: string` - текст сообщения об ошибке

3. Метод render
```
render(state: Partial<T> & IFormState): HTMLElement
```
Назначение: Обновляет состояние формы и отображает её. Обрабатывает валидность и ошибки через родительский класс, а затем устанавливает значения полей формы.
Параметры: `state: Partial<T> & IFormState` - объект состояния с флагами валидности, ошибками и значениями полей формы
...
```
Назначение: Обрабатывает изменения в полях ввода. Генерирует событие изменения поля в формате {formName}.{fieldName}:field-change.
...
## Класс Order
Назначение класса: Класс Order является компонентом для управления формой оформления заказа. Наследуется от Form<IOrderForm> и предоставляет функциональность для выбора способа оплаты через кнопки и управления адресом доставки через текстовое поле.
...
### Приватные свойства
```
private _payment?: paymentMethod;            // Текущий выбранный способ оплаты
private _address: string;                    // Текущее значение адреса доставки
```
...
```
Назначение: Инициализирует экземпляр класса Order. Вызывает конструктор родительского класса Form и настраивает обработчики для кнопок оплаты и поля адреса. Принимает параметрами: `container: HTMLFormElement` - HTML-элемент формы заказа, `events: IEvents` - экземпляр системы событий для коммуникации между компонентами.

### Приватные методы
1. Метод handlePaymentSelection
```
private handlePaymentSelection(method: paymentMethod): void
```
Назначение: Обрабатывает выбор способа оплаты пользователем. Вызывает метод onInputChange родительского класса для уведомления о изменении данных. Принимает параметром: `method: paymentMethod` - выбранный способ оплаты ("online" или "cash").

2. Метод setupPaymentHandlers
```
private setupPaymentHandlers(): void
```
Назначение: Настраивает обработчики кликов для кнопок выбора способа оплаты, связывая их с методом handlePaymentSelection.

3. Метод setupAddressHandler
```
private setupAddressHandler(): void
```
Назначение: Настраивает обработчик ввода для поля адреса доставки. Отслеживает изменения значения и вызывает метод onInputChange родительского класса.
...
```
Назначение: Устанавливает выбранный способ оплаты и обновляет визуальное состояние кнопок (добавляет/убирает класс button_alt-active). Принимает параметром `value: paymentMethod` - способ оплаты для установки ("online" или "cash").

2. Сеттер address
```
set address(value: string)
```
Назначение: Устанавливает значение адреса доставки во внутреннее состояние компонента. Принимает параметром: `value: string` - адрес для установки
...
## Класс Contacts
Назначение: Класс Contacts является компонентом для управления формой контактной информации при оформлении заказа. Наследуется от Form<IOrderForm> и предоставляет функциональность ввода email и телефона, делегируя валидацию и обработку ошибок родительскому классу.
...
```
Назначение: Устанавливает значение поля email. Принимает параметром: `value: string` - email для установки

2. Сеттер phone
```
set phone(value: string)
```
Назначение: Устанавливает значение поля телефона. Принимает параметром `value: string` - телефон для установки.
